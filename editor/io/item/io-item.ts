...
export class IoItem {
  ...
  static validateItemImport(itemName: ItemName, item: Record<string, number | null>, errors: string[]): void {
    ...
    IO.checkNumber(atk, `Item ${itemName} atk[${atk}] is invalid`, true, errors)
    const def = item[IoItem.ATTRIBUTE_DEF]
    IO.checkNumber(def, `Item ${itemName} def [${def}] is invalid`, true, errors)
    const hp = item[IoItem.ATTRIBUTE_HP]
    IO.checkNumber(hp, `Item ${itemName} hp [${hp}] is invalid`, true, errors)
    const expMulAdd = item[IoItem.ATTRIBUTE_EXP_MUL_ADD]
    IO.checkNumber(expMulAdd, `Item ${itemName} exp mul add [${expMulAdd}] is invalid`, true, errors)
    const expMulMuk = item[IoItem.ATTRIBUTE_EXP_MUL_MUL]
    IO.checkNumber(expMulMuk, `Item ${itemName} exp mul mul [${expMulMuk}] is invalid`, true, errors)
    const hpMulAdd = item[IoItem.ATTRIBUTE_HP_MUL_ADD]
    IO.checkNumber(hpMulAdd, `Item ${itemName} hp mul add [${hpMulAdd}] is invalid`, true, errors)
    const hpMulMuk = item[IoItem.ATTRIBUTE_HP_MUL_MUL]
    IO.checkNumber(hpMulMuk, `Item ${itemName} hp mul mul [${hpMulMuk}] is invalid`, true, errors)
  }

  static validateItemExport(itemName: ItemName, item: Item, errors: string[]): void {
    IO.checkNumber(item.atk, `Item ${itemName} atk [${item.atk}] is invalid`, true, errors)
    IO.checkNumber(item.def, `Item ${itemName} def [${item.def}] is invalid`, true, errors)
    IO.checkNumber(item.hp, `Item ${itemName} hp [${item.hp}] is invalid`, true, errors)
    IO.checkNumber(item.expMulAdd, `Item ${itemName} exp mul add [${item.expMulAdd}] is invalid`, true, errors)
    IO.checkNumber(item.expMulMul, `Item ${itemName} exp mul mul [${item.expMulMul}] is invalid`, true, errors)
  }
}
