...
export class IoRoomFromAttributes {
  static fromAttributes(
    ...
    if (Array.isArray(tiles)) {
      for (
        let lineIndex = 0;
        lineIndex < TILES_IN_ROW && lineIndex < tiles.length;
        lineIndex++
      ) {
        ...
        if (Array.isArray(tilesLine)) {
          for (
            let columnIndex = 0;
            columnIndex < TILES_IN_ROW && columnIndex < tilesLine.length;
            columnIndex++
          ) {
            const tileAttributes = tilesLine[columnIndex]
            const possibleTile = IoRoomFromAttributes.createTile(
              tileAttributes,
              enemies,
            )
            console.debug(
  ...
  private static createTile(
    ...
    const tileType: TileType = findEnum(
      TILE_TYPES,
      tile[IoRoom.ATTRIBUTE_TYPE] as string,
    )
    if (tileType != null) {
      switch (tileType) {
        case TileType.door:
  ...
  private static createTileStaircase(
    tile: Record<string, string | number>,
  ): Tile {
  ...
  private static createTileScore(tile: Record<string, string | number>): Tile {
    const scoreType: ScoreType = findEnum(
      SCORE_TYPES,
      tile[IoRoom.ATTRIBUTE_SCORE] as string,
    )
  ...
  private static createTileKey(tile: Record<string, string | number>): Tile {
    const keyColor: Color = findEnum(
      COLORS,
      tile[IoRoom.ATTRIBUTE_COLOR] as string,
    )
  ...
  private static createTileItem(tile: Record<string, string | number>): Tile {
    const itemName: Item = findEnum(
      ITEMS,
      tile[IoRoom.ATTRIBUTE_NAME] as string,
    )
  ...
  private static createTileEnemy(
    tile: Record<string, string | number>,
    enemies: Enemy[],
  ): Tile {
    const enemyType: EnemyType = findEnum(
      ENEMY_TYPES,
      tile[IoRoom.ATTRIBUTE_ENEMY_TYPE] as string,
    )
    ...
    const enemy = enemies.find(
      (e) => e.type == enemyType && e.level === enemyLevel,
    )
    if (enemy == null) {
      console.error(
        "IoRoomFromAttributes",
  ...
  private static createTileDoor(tile: Record<string, string | number>): Tile {
    const doorColor: Color = findEnum(
      COLORS,
      tile[IoRoom.ATTRIBUTE_COLOR] as string,
    )
