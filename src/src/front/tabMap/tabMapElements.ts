...
export class TabMapElements {
    ...
    constructor(editor: Editor) {
        this.editor = editor
        this.editor.eventManager.registerTileSelection((selectedTile, updateElementTree) => this.tileSelected(selectedTile, updateElementTree))
    }
    ...
    hole(): Hole {
        ...
        const items: Hole[] = ITEMS.map(i => html`
            <sl-tree-item data-type="${TileType.item}" data-name="${i.valueOf()}">${i}</sl-tree-item>`)

        const staircases: Hole[] = STAIRCASE_DIRECTIONS.map(s => html`
            <sl-tree-item data-type="${TileType.staircase}" data-direction="${s.valueOf()}">Staircase ${s}
            </sl-tree-item>`)

        const scores: Hole[] = SCORE_TYPES.map(s => html`
            <sl-tree-item data-type="${TileType.score}" data-score="${s.valueOf()}">Score ${s}</sl-tree-item>`)
        ...
        <sl-tree id="${TabMapElements.treeId}" selection="leaf" @sl-selection-change="${this.selectionChanged}">
            ...
            <sl-tree-item id="${TabMapElements.enemiesSubTreeId}">Enemy
            </sl-tree-item>
            <sl-tree-item>Key
                ${keys}
            </sl-tree-item>
    ...
    render(): void {
        ...
        const enemies: Hole[] = this.editor.tower.enemies.map((enemy: Enemy) => {
            ...
            return html`
                <sl-tree-item data-type="${TileType.enemy.valueOf()}"
                              data-enemy-type="${enemy.type ? enemy.type.valueOf() : ''}"
                              data-enemy-level="${enemy.level ? enemy.level : ''}">${enemyName}
                </sl-tree-item>`
        })
    ...
    private findTreeItem(tile: Tile): SlTreeItem {
        switch (tile.getType()) {
            ...
            case TileType.startingPosition:
                return this.findTreeItemFromValue({type: TileType.startingPosition.valueOf()})
            case TileType.wall:
